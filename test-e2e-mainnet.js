#!/usr/bin/env node
/**
 * E2E Test — Full StacksTasker Task Lifecycle on MAINNET (standalone, no browser)
 *
 * Drives the entire lifecycle via API calls — no relay/browser needed.
 * Uses REAL mainnet STX for payment (1 STX bounty).
 *
 * Flow:
 *   1. Register agents
 *   2. LOBSTER creates a task (mainnet)
 *   3. MONKEY bids on the task
 *   4. LOBSTER accepts MONKEY's bid
 *   5. MONKEY starts + submits result
 *   6. LOBSTER pays MONKEY on-chain (real mainnet STX transfer)
 *   7. LOBSTER approves the task
 *   8. LOBSTER reviews MONKEY
 *
 * Usage:
 *   node test-e2e-mainnet.js
 */
...
const BOUNTY = 1; // 1 STX — real money

// ── Wallet Loading (convert to mainnet addresses) ────────────
...
function loadWallets() {
  ...
  for (const file of files) {
    ...
    // Convert to mainnet address
    const [, hash] = c32addressDecode(data.address);
    data.mainnetAddress = c32address(22, hash);
...
function authHeaders(wallet) {
  return {
    ...
    'X-Wallet-Address': wallet.mainnetAddress,
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════');
  console.log('  StacksTasker E2E Test — MAINNET');
  console.log('═══════════════════════════════════════════');
  console.log(`  API: ${API_URL}`);
  console.log(`  Bounty: ${BOUNTY} STX (REAL MAINNET)`);

  const W = loadWallets();
  console.log(`  LOBSTER: ${W.LOBSTER.mainnetAddress}`);
  console.log(`  MONKEY:  ${W.MONKEY.mainnetAddress}`);

  // Check LOBSTER balance
  const lobBal = await checkBalance(W.LOBSTER.mainnetAddress);
  console.log(`  LOBSTER balance: ${lobBal} STX`);
  if (lobBal < BOUNTY + 0.5) {
    throw new Error(`LOBSTER needs at least ${BOUNTY + 0.5} STX, has ${lobBal}`);
  }

  // 1. Register agents
  step('Register agents on StacksTasker (mainnet)');
  for (const [name, w] of [['LOBSTER', W.LOBSTER], ['MONKEY', W.MONKEY]]) {
    try {
      const r = await api('POST', '/agents/register', w, {
        name: w.label || name,
        walletAddress: w.mainnetAddress,
        bio: `${name} agent`,
  ...
  const monkeyAgent = agentsList.find(a => a.walletAddress === W.MONKEY.mainnetAddress);
  const monkeyId = monkeyAgent ? (monkeyAgent.id || monkeyAgent.agentId) : W.MONKEY.mainnetAddress;
  ...
  const taskData = {
    title: `E2E Mainnet Test ${Date.now()}`,
    description: 'Automated E2E mainnet test — summarize the benefits of decentralized task markets in 2-3 sentences.',
    category: 'coding',
    bounty: BOUNTY,
    posterAddress: W.LOBSTER.mainnetAddress,
    network: 'mainnet',
  };
  ...
  try {
    await api('POST', `/tasks/${taskId}/messages`, W.MONKEY, { senderAddress: W.MONKEY.mainnetAddress, body: result });
  ...
  step(`LOBSTER pays MONKEY — ${BOUNTY} STX on-chain (MAINNET)`);
  let paymentTxId = null;
  const balBefore = await checkBalance(W.MONKEY.mainnetAddress);
  info(`MONKEY balance before: ${balBefore} STX`);
  try {
    paymentTxId = await transferSTX(W.LOBSTER.privateKey, W.MONKEY.mainnetAddress, BOUNTY);
    ok(`STX transfer broadcast — txid: ${paymentTxId}`);
    info(`${BOUNTY} STX → ${W.MONKEY.mainnetAddress}`);
    info(`Explorer: https://explorer.hiro.so/txid/${paymentTxId}?chain=mainnet`);
  } catch (e) {
  ...
  const approveBody = { posterAddress: W.LOBSTER.mainnetAddress };
  ...
  await api('POST', `/agents/${encodeURIComponent(monkeyId)}/review`, W.LOBSTER, {
    ...
    reviewerAddress: W.LOBSTER.mainnetAddress,
  });
  ok('Review submitted');

  // Balances after
  const lobAfter = await checkBalance(W.LOBSTER.mainnetAddress);
  const monkAfter = await checkBalance(W.MONKEY.mainnetAddress);
  info(`LOBSTER balance after: ${lobAfter} STX`);
  info(`MONKEY balance after: ${monkAfter} STX`);

  // Summary
  const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
  console.log('\n═══════════════════════════════════════════');
  console.log(`  E2E MAINNET TEST PASSED — ${elapsed}s`);
  console.log('═══════════════════════════════════════════\n');
}

main().catch(e => {
  console.error(`\n  E2E MAINNET TEST FAILED: ${e.message}\n`);
  process.exit(1);
});
